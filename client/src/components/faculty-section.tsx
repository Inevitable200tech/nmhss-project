...
export default function FacultySection() {
  const { data: section, isLoading } = useQuery<FacultySection>({
  ...
  const profiles = section?.profiles?.length ? section.profiles : fallbackProfiles;
  ...
  return (
    <section id="faculty" className="py-20 bg-card">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16" data-aos="fade-up">
          {stats.map((stat, index) => (
            <div key={index} className="bg-background p-8 rounded-xl shadow-lg border border-border text-center hover-lift" >
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${index === 0 ? "bg-primary" : index === 1 ? "bg-secondary" : "bg-accent"
                  }`}
              >
        ...
        {/* Department Structure (unchanged) */}
        <div className="mb-16">
          <h3
        ...
        <div className="bg-muted p-8 md:p-12 rounded-2xl">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {profiles.map((profile, index) => (
              <div key={index} className="bg-card p-6 rounded-xl shadow-lg border border-border text-center hover-lift">
                <img
                  src={profile.imageUrl || fallbackImages[index % fallbackImages.length]}
                  alt={profile.name}
